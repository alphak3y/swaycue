...
pub fn sin(ref mut _angle: U128) -> (U128, bool) {
    //represents the value 3141592653589793238
    let PI = U128::from(0xFFFFFFEE, 0xD6918000);
    let TWO_PI = U128::from(0,2) * PI;

    let INDEX_WIDTH        = U128::from(0,8);
    ...
    let interp     = (_angle >> INTERP_OFFSET) & ((ONE_U128 << INTERP_WIDTH) - ONE_U128);
    let mut index  = (_angle >> INDEX_OFFSET)  & ((ONE_U128 << INDEX_WIDTH)  - ONE_U128);

    let is_odd_quadrant: bool      = (_angle & QUADRANT_LOW_MASK)  == 0;
    let is_negative:     bool      = if (_angle & QUADRANT_HIGH_MASK) != 0 { true } else { false } ;
    ...
    offset1_2 = (index + 2);
    x1 = U128::from(0, table[index] as u64);
    x2 = U128::from(0, table[index + 1] as u64);

    //finish linear approximation of sine value
    let approximation = (U128::from(0,x2 - x1) * interp) >> INTERP_WIDTH;
    let sine = if is_odd_quadrant { x1 + approximation } else { x2 - approximation };
    ...
    sine = (sine * U128::from(0, 1e18) / U128::from(2_147_483_647));
    
    (sine, is_negative)
}

pub fn cos() -> (Q64x64, bool) {
    //represents the value 3141592653589793238
    let PI = U128::from(0xFFFFFFEE, 0xD6918000);
    let PI_OVER_TWO = PI / U128::from(0,2);
